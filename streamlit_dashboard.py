    ...
    if source == "Combined":
        ...
        for plat in ['Reddit', 'YouTube', 'Instagram', 'Google News']:
            ...
            if 'date' in sub.columns and not sub['date'].isna().all():
                st.sidebar.write(f"- {plat}: {sub['date'].min().strftime('%B %d, %Y @ %I:%M:%S %p')} to {sub['date'].max().strftime('%B %d, %Y @ %I:%M:%S %p')}")
            else:
...
if source == "Combined" and 'source' in filtered_df.columns:
    ...
    post_summary = f"### 📊 Total Posts: {len(filtered_df):,} (Combined)<br>"
    post_summary += "<ul style='margin-top:0; font-size:1rem; padding-left: 1.2em;'>"
    ...
    post_summary += "</ul><div style='margin-bottom:0px;'></div>"
